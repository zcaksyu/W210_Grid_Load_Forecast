...
class ForecastHour():
    ...
    def __init__(self, nws_weather_dictionary):
        ...
        self.utc = datetime.datetime.fromisoformat(nws_weather_dictionary["startTime"])
    ...
    def end(self):
        ...
        return self.utc.astimezone() + datetime.timedelta(hours=1)
...
class ObservationHour():
    ...
    def naive_utc_to_local(self):
        ...
        naive = datetime.datetime.fromisoformat(self.raw_data["timestamp"])
...
def return_NYISO_Load():
    ...
    str_dt = datetime.datetime.now().strftime('%Y%m%d')
...
def return_solar_data(url_type):
    ...
    df['period_end']=pd.to_datetime(df['period_end'].str.split('.',expand=True)[0])
...
def format_inputs():
    ...
    current_time = t[-1]
    t = [ datetime.datetime.strptime(str(e),'%Y-%m-%dT%H:%M:%S.000000000') for e in t]
    t_f = [e + datetime.timedelta(minutes=90) for e in t]
    ...
    input_list = np.array([load,solar,temp,solar_f,temp_f,uw_v,ow_v,weekend_holiday])
